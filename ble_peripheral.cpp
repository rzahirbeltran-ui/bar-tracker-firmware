...
static constexpr uint16_t BATT_LEVEL_HANDLE =
...
// Cola de envío: una ranura, el send es no-bloqueante
static btstack_context_callback_registration_t s_send_req;
static uint8_t  s_send_buf[20];
...
static const uint8_t ADV_DATA[] = {
    0x02, 0x01, 0x06,
    0x0B, 0x09, 'B','a','r','T','r','a','c','k','e','r'
};

// ── ATT read callback — sólo sirve para la característica de batería
static uint16_t att_read_cb(hci_con_handle_t /*con*/, uint16_t attr,
                             uint16_t offset, uint8_t* buf, uint16_t /*buf_sz*/) {
    if (attr == BATT_LEVEL_HANDLE) {
        if (buf && offset == 0) buf[0] = s_battery_pct;
        return 1;
    }
    return 0;
}

// ── BTstack llama aquí cuando hay buffer libre para enviar
...
void BLEPeripheral::init(const char* /*device_name*/) {
    l2cap_init();
    sm_init();
    att_server_init(profile_data, att_read_cb, nullptr);
...
bool BLEPeripheral::send(const uint8_t* data, size_t len) {
    ...
    size_t n = len < sizeof(s_send_buf) ? len : sizeof(s_send_buf);
    memcpy(s_send_buf, data, n);
    s_send_len     = static_cast<uint16_t>(n);
    s_send_pending = true;
...
void BLEPeripheral::set_battery(uint8_t pct) {
    s_battery_pct = pct;
}
